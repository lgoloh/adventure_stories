...
Anything that violates these constraints is an error:
  ...
  - A page declaration (num@filename) for a given page 
  must appear before anything else related to that page (choices, now, or other things in Step 4).
  - Page declarations must appear in order.  That is, 
  the declaration for page 0 (0@N:...) must be first, then the next page declaration (1@N:...)
...
For the first part of this assignment, you should make the following:
    ...
    you should have at least two classes, and should make good use of C++'s STL.
